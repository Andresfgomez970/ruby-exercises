...
describe FindNumber do
  ...
  describe '#game_over?' do
    let(:correct_guessing) { double('correct_guessing', value: 8) }
    let(:guess) { 8 }
    subject(:game_guessing) { described_class.new(0, 9, correct_guessing, guess) }
    ...
    context 'when guess and random_number are not equal' do
      let(:bad_guess) { 9 }
      subject(:game_guessing) { described_class.new(0, 9, correct_guessing, bad_guess) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        ...
        expect(edge_case_game.min).to eq(edge_case_game.max) 
      end

      it 'does not update max' do
        edge_case_game.update_range
        expect(edge_case_game.min).to eq(8)
      end
    end
  end
end
