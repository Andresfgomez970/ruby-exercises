...
describe CaesarBreaker do
  ...
  describe '#create_decrypted_messages' do
    ...
    context 'when single message is "a"' do
      # check the ones before with functinality
      subject(:letter) { described_class.new('a', CaesarTranslator.new('b')) }
      it 'check list @decrypt_messages' do
        letter.create_decrypted_messages
        expect(subject.instance_variable_get(:@decrypted_messages) ).to eq(('a'..'z').to_a.rotate!)
      end
    end
