...
def count_empty_seats(chart)
  chart.flatten.reduce(0) {|sum, element| element == nil ? sum += 1: sum}
...
def find_favorite(array_of_hash_objects)
  ...
  array = array_of_hash_objects.select{|element| element.keys.include?(:is_my_favorite?) && element.values.include?(true)}[0]

  ## other way
  # array = array_of_hash_objects.select do |element|
  #   element.any?{|key, value| key == :is_my_favorite? && value == true}
  # end
  # array[0]
